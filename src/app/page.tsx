...
import { getDepartureTimes, isDataStale, RouteName, Direction } from "@/lib/schedule";
...
export default function Home() {
  ...
  useEffect(() => {
    const calculateNextFerry = () => {
      ...
      let departureTimes = getDepartureTimes(now, route.name, route.direction);
      ...
      if (!nextDeparture) {
        ...
        departureTimes = getDepartureTimes(tomorrow, route.name, route.direction);
    ...
    calculateNextFerry();
    const interval = setInterval(calculateNextFerry, 1000);
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center px-4 overflow-y-auto min-h-0">
        {stale && (
          <p className="text-xs mb-2" style={{ color: '#dc2626' }}>
            Schedule data may be outdated. Check halifax.ca for current times.
          </p>
        )}
